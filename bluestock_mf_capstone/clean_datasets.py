...
def clean_nav_history():
    ...
    dest_path = os.path.join(PROCESSED_DIR, "02_nav_history.csv")
    df_cleaned.to_csv(dest_path, index=False)
    # Also save as nav_history.csv in processed
    df_cleaned.to_csv(os.path.join(PROCESSED_DIR, "nav_history.csv"), index=False)
...
def clean_investor_transactions():
    ...
    # Check existing types: usually they are like 'sip', 'lumpsum', etc.
    ...
    dest_path = os.path.join(PROCESSED_DIR, "08_investor_transactions.csv")
    df.to_csv(dest_path, index=False)
    # Also save as investor_transactions.csv
    df.to_csv(os.path.join(PROCESSED_DIR, "investor_transactions.csv"), index=False)
...
def clean_scheme_performance():
    ...
    dest_path = os.path.join(PROCESSED_DIR, "07_scheme_performance.csv")
    df.to_csv(dest_path, index=False)
    # Also save as scheme_performance.csv
    df.to_csv(os.path.join(PROCESSED_DIR, "scheme_performance.csv"), index=False)
...
def process_remaining_files():
    ...
    # fund_master
    df_fm = pd.read_csv(os.path.join(RAW_DIR, "01_fund_master.csv"))
    df_fm.to_csv(os.path.join(PROCESSED_DIR, "01_fund_master.csv"), index=False)
    df_fm.to_csv(os.path.join(PROCESSED_DIR, "fund_master.csv"), index=False)
    ...
    df_aum.to_csv(os.path.join(PROCESSED_DIR, "aum_by_fund_house.csv"), index=False)
    ...
    df_sip.to_csv(os.path.join(PROCESSED_DIR, "monthly_sip_inflows.csv"), index=False)
    ...
    df_cat.to_csv(os.path.join(PROCESSED_DIR, "category_inflows.csv"), index=False)
    ...
    df_folio.to_csv(os.path.join(PROCESSED_DIR, "industry_folio_count.csv"), index=False)
    ...
    df_holdings.to_csv(os.path.join(PROCESSED_DIR, "portfolio_holdings.csv"), index=False)
    ...
    df_bench.to_csv(os.path.join(PROCESSED_DIR, "benchmark_indices.csv"), index=False)
